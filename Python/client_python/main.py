...
while bot.day <= bot.running_days:
    ...
    now = round(ConvertToSimTime_us(bot.start_time, bot.time_ratio, bot.day, bot.running_time))  # 四舍五入得当前时间戳
    ...
    for s in range(now, SimTimeLen + endWaitTime):
        ...
        if t < SimTimeLen - 30:
            bot.work()
    bot.eod()  # 每个交易日结束时会调用该函数，可用于每天交易结束时执行一些操作
    bot.day += 1
bot.final()  # 在所有交易日结束后会调用，可用于赛后处理一些工作和分析
